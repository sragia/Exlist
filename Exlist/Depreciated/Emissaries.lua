...
local function Updater(event)
    ...
    if trackedBounties == 3 then
        ...
        for i = 1, C_QuestLog.GetNumQuestLogEntries() do
            ...
            if isBounty and info.level >= Exlist.constants.MAX_CHARACTER_LEVEL then
                local _, _, _, current, total =
                    C_QuestLog.GetQuestObjectiveInfo(info.questID, 1, false)
...
local function Linegenerator(tooltip, data, character)
    ...
    if availableEmissaries > 0 then
        ...
        local sideTooltip = {
            title = WrapTextInColorCode(L["Available Emissaries"],
        ...
        for i = 1, #data do
            if data[i] and data[i].endTime > timeNow then
                table.insert(sideTooltip.body, {
                    data[i].name .. "(" ..
                        TimeLeftColor(data[i].endTime - timeNow, {36000, 72000}) ..
                        ")",
...
local function GlobalLineGenerator(tooltip, data)
    ...
    Exlist.AddLine(tooltip, {
        WrapTextInColorCode(L["Emissaries"], colors.sideTooltipTitle)
    }, 14)

    for questId, info in spairs(data or {}, function(t, a, b)
        return t[a].endTime < t[b].endTime
    end) do
        Exlist.AddLine(tooltip, {
            info.title, TimeLeftColor(info.endTime - timeNow, {36000, 72000})
        })
    end
end
...
local function init()
    Exlist.ConfigDB.settings.extraInfoToggles.emissary =
        Exlist.ConfigDB.settings.extraInfoToggles.emissary or
            {name = L["Emissaries"], enabled = true}
end
...
local data = {
    ...
    updater = Updater,
    event = {
        "QUEST_TURNED_IN", "PLAYER_ENTERING_WORLD", "QUEST_REMOVED",
