...
local function GetCharacterOrder()
    if not settings.reorder then return settings.characterOrder end
    local t = {}
...
local function GearTooltip(self, info)
    local geartooltip = QTip:Acquire("CharInf_GearTip", 7, "CENTER", "LEFT",
    ...
    local header = "|T" .. specIcon .. ":25:25|t " .. "|c" ..
                       RAID_CLASS_COLORS[info.class].colorStr .. info.name ..
                       "|r " .. (info.level or 0) .. ' level'
    local line = geartooltip:AddHeader()
    geartooltip:SetCell(line, 1, header, "LEFT", 3)
    geartooltip:SetCell(line, 7,
                        string.format("%i " .. L["ilvl"], (info.iLvl or 0)),
                        "CENTER")
    geartooltip:AddSeparator(1, .8, .8, .8, 1)
    line = geartooltip:AddHeader()
    geartooltip:SetCell(line, 1, WrapTextInColorCode(L["Gear"],
    ...
    if gear then
        for i = 1, #gear do
            ...
            if gear[i].enchant or gear[i].gem then
                if type(gear[i].gem) == 'table' then
                    if gear[i].enchant then
                        enchantements = string.format("|c%s%s|r",
                                                      colors.enchantName,
                                                      gear[i].enchant or "")
                    end
                    for b = 1, #gear[i].gem do
                        if enchantements ~= "" then
                            enchantements =
                                string.format("%s\n|T%s:20|t%s", enchantements,
                                              gear[i].gem[b].icon,
                                              gear[i].gem[b].name)
                        else
                            enchantements =
                                string.format("|T%s:20|t%s",
                                              gear[i].gem[b].icon,
                                              gear[i].gem[b].name)
                        end
                    end
            ...
            elseif hasEnchantSlot[gear[i].slot] then
                enchantements =
                    WrapTextInColorCode(L["No Enchant!"], "ffff0000")
            end
            local line = geartooltip:AddLine(gear[i].slot)
            geartooltip:SetCell(line, 2,
                                string.format("|c%s%-5d|r",
                                              Exlist.setIlvlColor(gear[i].ilvl),
                                              gear[i].ilvl or 0))
            geartooltip:SetCell(line, 3, string.format("|T%s:20|t %s", gear[i]
                                                           .itemTexture or "",
                                                       gear[i].itemLink or ""),
                                "LEFT", 2)
            geartooltip:SetFont(fontName, fontHeight and fontHeight - 2 or 10,
                                fontFlags)
    ...
    if info.professions and #info.professions > 0 then
        -- professsions
        line = geartooltip:AddHeader()
        geartooltip:SetCell(line, 1,
                            WrapTextInColorCode(L["Professions"], "ffffb600"),
                            "CENTER", 7)
        ...
        for i = 1, #p do
            ...
            geartooltip:SetCell(line, 1,
                                string.format("|T%s:20|t%s", p[i].icon,
                                              p[i].name), "LEFT")
            geartooltip:SetCell(line, 2, "", "LEFT", 5) -- spacer for status bar
            geartooltip:SetCell(line, 7,
                                string.format("|cff%s%s|r",
                                              Exlist.ProfessionValueColor(
                                                  p[i].curr, isArch), p[i].curr),
                                "CENTER")

            local statusBar = Exlist.AttachStatusBar(
                                  geartooltip.lines[line].cells[2])
            table.insert(geartooltip.statusBars, statusBar)
            statusBar:SetMinMaxValues(0, isArch and 800 or
                                          const.MAX_PROFESSION_LEVEL)
            statusBar:SetValue(p[i].curr)
            statusBar:SetWidth(tipWidth)
            statusBar:SetStatusBarColor(Exlist.ColorHexToDec(
                                            Exlist.ProfessionValueColor(
                                                p[i].curr, isArch)))
    ...
    geartooltip:SetBackdropBorderColor(c.borderColor.r, c.borderColor.g,
...
local function PopulateTooltip(tooltip)
        ...
        if t then
            if settings.horizontalMode then
                for module, info in pairs(t.modules) do
                    if not modulesAdded[module] and
                        (module ~= "_Header" and module ~= "_HeaderSmall") then
                        modulesAdded[module] =
                            {prio = info.priority, name = info.name}
                    end
                end
            else
                ...
                for i = 1, t.num do tooltip:AddLine() end
                if i ~= #charOrder then
    ...
    if settings.horizontalMode then
        ...
        tooltip:AddSeparator(1, 1, 1, 1, .85)
        -- Add Module Texts
        for module, info in Exlist.spairs(modulesAdded, function(t, a, b)
            return t[a].prio < t[b].prio
        end) do moduleLine[module] = tooltip:AddLine(info.name) end
    end
            ...
            if settings.horizontalMode then
                local headerText =
                    settings.shortenInfo and header.data[1].data .. " " ..
                        header.data[2].data or header.data[1].data ..
                        "             " .. header.data[2].data
                tooltip:SetCell(1, 1,
                                "|T" ..
                                    [[Interface/Addons/Exlist/Media/Icons/ExlistLogo2.tga]] ..
                                    ":" .. logoTexSize .. "|t", "CENTER")
                tooltip:SetCell(rowHeadNum - 1, headerCol, headerText, "CENTER",
                                4)
                tooltip:SetCellScript(rowHeadNum - 1, headerCol, "OnEnter",
                                      header.data[1].OnEnter,
                                      header.data[1].OnEnterData)
                tooltip:SetCellScript(rowHeadNum - 1, headerCol, "OnLeave",
                                      header.data[1].OnLeave,
                                      header.data[1].OnLeaveData)
            else
                tooltip:SetCell(rowHeadNum - 1, headerCol, header.data[1].data,
                                "LEFT", headerWidth)
                tooltip:SetCell(rowHeadNum - 1, headerCol + headerWidth,
                                header.data[2].data, "RIGHT")
                tooltip:SetLineScript(rowHeadNum - 1, "OnEnter",
                                      header.data[1].OnEnter,
                                      header.data[1].OnEnterData)
                tooltip:SetLineScript(rowHeadNum - 1, "OnLeave",
                                      header.data[1].OnLeave,
                                      header.data[1].OnLeaveData)
            end
            local smallHeader =
                Exlist.tooltipData[character].modules["_HeaderSmall"]
            tooltip:SetCell(rowHeadNum, headerCol, smallHeader.data[1].data,
                            justification, 4, nil, nil, nil, 2000,
                ...
                if module ~= "_HeaderSmall" and module ~= "_Header" then
                    ...
                    for i = 1, info.num do
                        ...
                        if data.pulseAnim then
                            local cell =
                                tooltip.lines[row].cells[col + offsetCol]
...
local function showTooltip(self)
    ...
    for i = 1, #charOrder do
        ...
        charData.name = name
        -- header
        local specIcon = charData.specId and iconPaths[charData.specId] or
                             iconPaths[0]
        local headerText, subHeaderText = "", ""
        if settings.shortenInfo and charData.class then

            headerText = "|c" .. RAID_CLASS_COLORS[charData.class].colorStr ..
                             name .. "|r "
            subHeaderText = string.format("|c%s%s", colors.sideTooltipTitle,
                                          realm)

        elseif (charData.class) then
            headerText = "|T" .. specIcon .. ":25:25|t " .. "|c" ..
                             RAID_CLASS_COLORS[charData.class].colorStr .. name ..
                             "|r "
            subHeaderText = string.format("|c%s%s - " .. L["Level"] .. " %i",
                                          colors.sideTooltipTitle, realm,
                                          charData.level)
        end
        -- Header Info
        Exlist.AddData({
        ...
        for _, data in ipairs(mDB.lineGenerators) do
            if settings.allowedModules[data.key].enabled and data.type == "main" then
                xpcall(data.func, geterrorhandler(), tooltip,
                       charData[data.key], character)
            end
        end
    end
    -- Add Data to tooltip
