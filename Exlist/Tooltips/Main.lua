...
local function GearTooltip(self,info)
  ...
  if info.professions and #info.professions > 0 then
    ...
    for i=1,#p do
      ...
      geartooltip:SetCell(line,7,string.format("|cff%s%s|r",Exlist.ProfessionValueColor(p[i].curr,isArch),p[i].curr),"CENTER")
...
local function PopulateTooltip(tooltip)
  ...
  if settings.horizontalMode then
    ...
    tooltip:AddSeparator(1, 1, 1, 1, .85)
    -- Add Module Texts
    for module,info in Exlist.spairs(modulesAdded,function(t,a,b) return t[a].prio<t[b].prio end) do
  ...
  for i=1,#charOrder do
    ...
    if Exlist.tooltipData[character] then
      ...
      local offsetRow = 0
      local row = 0
      for module,info in Exlist.spairs(Exlist.tooltipData[character].modules,function(t,a,b) return t[a].priority<t[b].priority end) do
...
local function configureTooltip(self, tooltip)
  ...
  tooltip.time = 0
  tooltip.elapsed = 0
  -- TODO: make this scale for multiple tooltips
