...
local function Linegenerator(tooltip, data, character)
  ...
  for index = 1, #raidOrder do
    if data[raidOrder[index]] then
      -- Raid
      info.priority =
        prio + (index / 100) + ((20 - raids[raidOrder[index]].expansion) + 50 - raids[raidOrder[index]].order) / 1000
      local added = false
      local cellIndex = 1
      local line
      for difIndex = 1, #diffOrder do
        ...
        if raidInfo and raidInfo.locked then
          ...
          local sideTooltipTable = {
            title = WrapTextInColorCode(raidOrder[index] .. " (" .. diffOrder[difIndex] .. ")", colors.sideTooltipTitle),
            body = {}
          }
