...
local C_TaskQuest, C_WorldMap, EJ_GetCreatureInfo, C_ContributionCollector,
      C_Timer = C_TaskQuest, C_WorldMap, EJ_GetCreatureInfo,
                C_ContributionCollector, C_Timer
local pairs, ipairs, time, select = pairs, ipairs, time, select
local GetTime = GetTime
local IsInRaid, IsInInstance = IsInRaid, IsInInstance
local GetCurrentMapAreaID, SetMapByID, GetMapNameByID = GetCurrentMapAreaID,
                                                        SetMapByID,
                                                        GetMapNameByID
local GetNumMapLandmarks, GetMapLandmarkInfo = GetNumMapLandmarks,
                                               GetMapLandmarkInfo
...
local function GetWarfrontEnd(warfront)
    ...
    local state, pctComplete, timeNext, timeStart =
        C_ContributionCollector.GetState(warfronts[warfront][faction])
    if state == 2 then
    ...
    elseif state == 1 and pctComplete < 1 then
        ...
        state, pctComplete, timeNext, timeStart =
            C_ContributionCollector.GetState(
                warfronts[warfront][OpossiteFacton(faction)])
        if state == 1 then return {value = pctComplete, type = 'pct'} end
    end
end
...
local function GetWarfrontStatus()
    ...
    for wf, ids in pairs(warfronts) do
        ...
        for faction, id in pairs(ids) do
            local state, pctComplete, timeNext, timeStart =
                C_ContributionCollector.GetState(id)
            local appearanceData =
                C_ContributionCollector.GetContributionAppearance(id, state)
            local name = C_ContributionCollector.GetName(id)
            t[wf][faction] = {
                name = name,
                faction = faction,
                state = state,
                stateName = appearanceData and appearanceData.stateName or
                    L["Contributing"],
...
local function Updater(e, info)
    if e == "WORLD_QUEST_SPOTTED" and #info > 0 then
        ...
        local t = Exlist.GetCharacterTableKey((GetRealmName()),
                                              (UnitName('player')), key)
        ...
        for _, wq in ipairs(info) do
            ...
            if defaultInfo then
                local endTime = defaultInfo.warfront and
                                    GetWarfrontEnd(defaultInfo.warfront) or
                                    wq.endTime
                t[wq.questId] = {
        ...
        return
    elseif not (IsPlayerAtEffectiveMaxLevel()) or GetTime() - lastUpdate < 5 or
        IsInRaid() or select(2, IsInInstance()) ~= "none" then
...
local function Linegenerator(tooltip, data, character)
    ...
    for spellId, info in pairs(data) do
        ...
        table.insert(strings, {
            string.format("%s (%s)", info.name,
                          info.endTime and
                              (type(info.endTime) == 'table' or info.endTime >
                                  timeNow) and FormatEndTime(info.endTime) or
                              WrapTextInColorCode(L["Not Available"],
                                                  colors.notavailable)),
            info.defeated and
                WrapTextInColorCode(L["Defeated"], colors.completed) or
    ...
    if availableWB > 0 then
        local sideTooltip = {
            body = strings,
            title = WrapTextInColorCode(L["World Bosses"],
        ...
        local info = {
            ...
            moduleName = key,
            priority = prio,
            titleName = WrapTextInColorCode(L["World Bosses"] .. ":",
                                            colors.faded),
...
local function GetWFCurrentStatus(wf)
    ...
    for f, data in pairs(wf) do
        if faction ~= f then
            ...
            if data.state == 1 or data.state == 2 then tmp = data end
        end
    end
    ...
    return tmp.name, stateName, tmp.timeNext and tmp.timeNext - time(),
           tmp.contributed
end
...
local function GlobalLineGenerator(tooltip, data)
    local timeNow = time()
    if not data then return end
    if data.worldbosses and
        Exlist.ConfigDB.settings.extraInfoToggles.worldbosses.enabled then
        local added = false
            ...
            if type(info.endTime) == 'table' or info.endTime > timeNow then
                if not added then
                    ...
                    Exlist.AddLine(tooltip, {
                        WrapTextInColorCode(L["World Bosses"],
                                            colors.sideTooltipTitle)
                    }, 14)
                end
    ...
    if data.warfronts and
        Exlist.ConfigDB.settings.extraInfoToggles.warfronts.enabled then
        Exlist.AddLine(tooltip, {
        ...
        for wf, wfData in pairs(data.warfronts) do
            ...
            Exlist.AddLine(tooltip, {
                name, stateName,
                pct < 1 and string.format("%.1f%%", pct * 100) or
                    Exlist.TimeLeftColor(timeLeft)
            })
        end
    end
end
...
local function init()
    local t = {}
    for questId in pairs(worldBossIDs) do t[#t + 1] = questId end
    Exlist.RegisterWorldQuests(t, true)
    Exlist.ConfigDB.settings.extraInfoToggles.worldbosses =
        Exlist.ConfigDB.settings.extraInfoToggles.worldbosses or
            {name = L["World Bosses"], enabled = true}
    Exlist.ConfigDB.settings.extraInfoToggles.warfronts =
        Exlist.ConfigDB.settings.extraInfoToggles.warfronts or
            {name = L["Warfronts"], enabled = true}
    -- BFA Prepatch Retire
    ...
    if gt.worldbosses and gt.worldbosses.argus then
        ...
        for _, quests in pairs(gt.worldbosses) do
            for questId, info in pairs(quests) do t[questId] = info end
        end
        gt.worldbosses = t
        Exlist.UpdateChar(key, gt, "global", "global")
    end
...
local data = {
    ...
    updater = Updater,
    event = {
        "PLAYER_ENTERING_WORLD", "EJ_DIFFICULTY_UPDATE",
        "PLAYER_ENTERING_WORLD_DELAYED", "WORLD_QUEST_SPOTTED"
    },
