...
local table, pairs, ipairs, type, math, time, GetTime, string, tonumber, print =
    table, pairs, ipairs, type, math, time, GetTime, string, tonumber, print
local C_TaskQuest = C_TaskQuest
local GetQuestLogRewardInfo, GetNumQuestLogRewardCurrencies,
      GetQuestLogRewardCurrencyInfo, GetQuestLogRewardMoney =
    GetQuestLogRewardInfo, GetNumQuestLogRewardCurrencies,
    GetQuestLogRewardCurrencyInfo, GetQuestLogRewardMoney
local GetCurrentMapAreaID, SetMapByID, ToggleWorldMap = GetCurrentMapAreaID,
                                                        SetMapByID,
                                                        ToggleWorldMap
local GetCurrencyInfo, GetSpellInfo, GetItemSpell = GetCurrencyInfo,
                                                    GetSpellInfo, GetItemSpell
local BonusObjectiveTracker_TrackWorldQuest =
...
local function GetQuestRewards(questId)
    ...
    for _, currencyReward in
        ipairs(C_QuestLog.GetQuestRewardCurrencies(questId)) do
...
local function compare(current, target, comp)
    ...
    target = target / 1000
    local ret = loadstring(string.format("return %f %s %f", current, comp,
                                         target))
    return ret()
end
    ...
    for i, reward in ipairs(rewards) do
        if rules[reward.type] and rules[reward.type][reward.name] then
            ...
            if reward.type == "money" then
                verdict, ruleId, targetReward =
                    compare(reward.amount.gold, rule.amount, rule.compare),
                    rule.id, i
            else
                verdict, ruleId, targetReward =
                    compare(reward.amount, rule.amount, rule.compare), rule.id,
                    i
            end
        end
    end
...
function Exlist.ScanQuests()
    ...
    for index, zoneId in ipairs(zones) do
        local wqs = C_TaskQuest.GetQuestsOnMap(zoneId)
        for _, info in pairs(wqs or {}) do
            local timeLeft =
                C_TaskQuest.GetQuestTimeLeftMinutes(info.questID) or 0
            local rewards = GetQuestRewards(info.questID)
            local checkRules, ruleid, targetReward = CheckRewardRules(rewards)
            if (trackedQuests[info.questID] and
                trackedQuests[info.questID].enabled) or checkRules then
            ...
            if timeLeft == 0 then timeLeft = 5 end
            tl = tl > timeLeft and timeLeft or tl
        end
    end
    -- Rescan Scheduling
    Exlist.Debug("Rescan Scheduled in:", tl, "minutes")
    if rescanTimer then timer:CancelTimer(rescanTimer) end
    rescanTimer = timer:ScheduleTimer(Exlist.ScanQuests, (60 * tl + 30) / 2)

    -- Send Data
    if #rt > 0 then Exlist.SendFakeEvent("WORLD_QUEST_SPOTTED", rt) end
end

local function RemoveExpiredQuest(questId)
    local gt = Exlist.GetCharacterTableKey("global", "global", key)
    for _, wqs in pairs(gt) do wqs[questId] = nil end
    Exlist.UpdateChar(key, gt, "global", "global")
end
...
local function GetFormatedRewardString(r, noColor)
    ...
    if r.name == "Gold" then
        if noColor then
            s = r.amount.gold .. "g " .. r.amount.silver .. "s " ..
                    r.amount.coppers .. "c"
        else
            s = r.amount.gold .. "|cFFd8b21ag|r " .. r.amount.silver ..
                    "|cFFadadads|r " .. r.amount.coppers .. "|cFF995813c|r"
        end
    else
        if r.amount > 1 then
...
local function Updater(event, questInfo)
    if event == "PLAYER_ENTERING_WORLD" and UnitLevel("player") >=
        Exlist.constants.MAX_CHARACTER_LEVEL then
...
local function GlobalLineGenerator(tooltip, data)
    ...
    if data and Exlist.ConfigDB.settings.extraInfoToggles.worldquests.enabled then
        ...
        local faction = UnitFactionGroup("player")
        local first = true
        for questId, info in spairs(data[faction] or {}, function(t, a, b)
            return t[a].endTime < t[b].endTime
        end) do
            if info.endTime < timeNow or
                (wq[questId] and not wq[questId].enabled) then
                RemoveExpiredQuest(questId)
            else
                if first then
                    Exlist.AddLine(tooltip, {
                        WrapTextInColorCode(L["World Quests"],
                ...
                local timeLeft = Exlist.TimeLeftColor(info.endTime - timeNow,
                                                      {3600, 14400})
                local targetReward = ""
                local sideTooltip = {
                    title = WrapTextInColorCode(info.name, colors.questTitle),
                    body = {
                        L["Time Left: "] .. timeLeft,
                        WrapTextInColorCode(L["Rewards"], colors.questTitle)
                    }
                }
                ...
                for i, reward in ipairs(info.rewards) do
                    ...
                    table.insert(sideTooltip.body,
                                 GetFormatedRewardString(reward))
                end
                if targetReward == "" then
                    targetReward =
                        GetFormatedRewardString(info.rewards[1], true)
                end

                local lineNum = Exlist.AddLine(tooltip, {
                    AddCheckmark(info.name, C_QuestLog.IsQuestFlaggedCompleted(
                                     info.questId)), timeLeft,
                    WrapTextInColorCode(string.format("%s  - %s", targetReward,
                                                      C_Map.GetMapInfo(
                                                          info.zoneId).name or
                                                          ""), colors.faded)
                })
                ...
                Exlist.AddScript(tooltip, lineNum, nil, "OnEnter",
                                 Exlist.CreateSideTooltip(), sideTooltip)
                Exlist.AddScript(tooltip, lineNum, nil, "OnLeave",
                                 Exlist.DisposeSideTooltip())
            end
        end
    end
end
...
local function SetupWQConfig(refresh)
    ...
    local options = {
        ...
        args = {
            ...
            forceRefresh = {
                ...
                func = function() Exlist.ScanQuests() end
            },
            itemInput = {
                type = "input",
                order = 1.5,
                ...
                set = function(self, v)
                    ...
                    if name then
                        ...
                        print(Exlist.debugString, L["Invalid World Quest ID:"],
                              v)
                    end
                end,
                width = "full"
    ...
    for questID, info in pairs(wq) do
        ...
        o[questID .. "enabled"] = {
            ...
            set = function(self, v) info.enabled = v end
        }
        n = n + 1
        if not info.rewards or #info.rewards < 1 then
            info.rewards = GetQuestRewards(questID)
        ...
        get = function()
            if tmpConfigRule.rewardName == "" then
                tmpConfigRule.rewardName =
                    rewardRules.DEFAULT[tmpConfigRule.ruleType].defaultValue
            end
    ...
    options.args["WQRulesAmount"] = {
        ...
        width = 0.6,
        name = "",
        get = function() return tostring(tmpConfigRule.amount) end,
        set = function(_, v)
    ...
    options.args["WQRulesSaveBtn"] = {
        ...
        name = L["Save"],
        func = function()
            local name = rewardRules.DEFAULT[tmpConfigRule.ruleType]
                             .customFieldValue == tmpConfigRule.rewardName and
                             tmpConfigRule.customReward or
                             tmpConfigRule.rewardName
            SetQuestRule(name, tmpConfigRule.ruleType, tmpConfigRule.amount,
                         tmpConfigRule.compareValue)
            Exlist.ScanQuests()
            SetupWQConfig(true)
        end
    }
    ...
    -- for custom rewards
    if rewardRules.DEFAULT[tmpConfigRule.ruleType].useCustom and
        rewardRules.DEFAULT[tmpConfigRule.ruleType].customFieldValue ==
        tmpConfigRule.rewardName then
        n = n + 1
        options.args["WQRulesCustomName"] = {
            ...
            get = function() return tmpConfigRule.customReward or "" end,
            set = function(_, v)
                    ...
                    StaticPopupDialogs["DeleteWQRuleDataPopup_" .. rewardName] =
                        {
                            ...
                            OnAccept = function(self)
                                StaticPopup_Hide(
                                    "DeleteWQRuleDataPopup_" .. rewardName)
...
local function init()
    ...
    for key in pairs(gt) do
        if key ~= "Alliance" and key ~= "Horde" then gt[key] = nil end
    end
    Exlist.UpdateChar(key, gt, "global", "global")
    ...
    Exlist.ConfigDB.settings.extraInfoToggles.worldquests = Exlist.ConfigDB
                                                                .settings
                                                                .extraInfoToggles
                                                                .worldquests or
                                                                {
