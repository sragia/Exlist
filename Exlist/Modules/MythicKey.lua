...
local GetContainerNumSlots, GetContainerItemLink = GetContainerNumSlots,
                                                   GetContainerItemLink
local ItemRefTooltip, UIParent, ShowUIPanel = ItemRefTooltip, UIParent,
                                              ShowUIPanel
...
local function Updater(event)
    ...
    if affixes then
        ...
        for i, affixLevel in ipairs(affixThreshold) do
            ...
            if (affixes[i]) then availableAffixes[i] = affixes[i].id end
        end
        ...
        local t = {
            ...
            itemLink = string.format(
                ...
                158923, challengeMapID, keyLevel, availableAffixes[1],
                availableAffixes[2], availableAffixes[3], availableAffixes[4],
                L["Keystone"], mapName, keyLevel)
        }
...
local function Linegenerator(tooltip, data, character)
    ...
    local dungeonName = settings.shortenInfo and Exlist.ShortenedMPlus[mapId] or
                            data.dungeon
    local info = {
        data = WrapTextInColorCode("[" .. dungeonName .. " +" .. data.level ..
                                       "]", colors.mythicplus.key),
        character = character,
        moduleName = key,
        priority = prio,
    ...
    for i = 1, #data do
        if not added then
            Exlist.AddLine(tooltip, {
                WrapTextInColorCode(L["Mythic+ Affixes"],
                                    colors.sideTooltipTitle)
            }, 14)
            added = true
        end
        local line = Exlist.AddLine(tooltip, {
            string.format("|T%s:15|t %s %s", data[i].icon or unknownIcon,
                          data[i].name or L["Unknown"],
                          WrapTextInColorCode(
                              string.format("- %s %i+", L["Level"],
                                            affixThreshold[i]), colors.faded))
        })
        if data[i].desc then
...
local function init()
    Exlist.ConfigDB.settings.extraInfoToggles.affixes =
        Exlist.ConfigDB.settings.extraInfoToggles.affixes or
            {name = L["Mythic+ Weekly Affixes"], enabled = true}
