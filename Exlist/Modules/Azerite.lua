...
local Exlist = Exlist
local L = Exlist.L
local C_AzeriteItem, C_AzeriteEmpoweredItem = C_AzeriteItem,
...
local function GetAvailableTraits(powerLevel)
    ...
    for _, slot in next, {1, 3, 5} do
        ...
        if not item:IsItemEmpty() then
            ...
            if C_AzeriteEmpoweredItem.IsAzeriteEmpoweredItem(itemLoc) and
                C_AzeriteEmpoweredItem.HasAnyUnselectedPowers(itemLoc) then
                ...
                for _, tierInfo in ipairs(tiers) do
                    if tierInfo.unlockLevel <= powerLevel then
                        ...
                        for _, powerId in ipairs(tierInfo.azeritePowerIDs) do
                            ...
                            if C_AzeriteEmpoweredItem.IsPowerSelected(itemLoc,
                                                                      powerId) then
                                empty = false
                                break
                            end
...
local function Updater(event)
    ...
    if UnitLevel("player") == 120 then
        ...
        t.weekly = {completed = completed, curr = curr, max = max}
    end
    Exlist.UpdateChar(key, t)
end

local slotNames = {[1] = L["Head"], [3] = L["Shoulders"], [5] = L["Chest"]}
...
local function Linegenerator(tooltip, data, character)
    ...
    local info = {
        ...
        data = format("|c%s%s:|r %i - %.1f%%", colors.faded, L["Level"],
                      data.powerLevel, (data.xp / data.maxXp) * 100),
        OnEnter = Exlist.CreateSideTooltip(),
        OnEnterData = {
            title = WrapTextInColorCode(L["Azerite Power"],
                                        colors.sideTooltipTitle),
            body = {
                {
                    L["Progress"],
                    format("%.1f%% (%i/%i)", (data.xp / data.maxXp) * 100,
                           data.xp, data.maxXp)
    ...
    for slot, availableTraits in pairs(data.traitsAvailable) do
        tinsert(info.OnEnterData.body, {
            format("%s: %s", slotNames[slot],
                   WrapTextInColorCode(format("%i |4 %s: %s; %s",
                                              availableTraits, L["trait"],
                                              L["traits"], L["available"]),
                                       colors.available))
        })
        total = total + availableTraits
    end
    if total > 0 then
        info.data = info.data .. WrapTextInColorCode(
                        format(" %i %s", total,
                               shorten and L["avail"] or L["available"]),
                        colors.available)
    end
    Exlist.AddData(info)
    -- Weekly Info
    if data.weekly and Exlist.ConfigDB.settings.azeriteWeekly then
        ...
        elseif data.weekly.curr >= data.weekly.max then
            ...
            weekly.data = string.format("%s/%s - %.1f%%",
                                        ...
                                        (data.weekly.curr / data.weekly.max) *
                                            100)
        end
...
local function Modernize(data)
    ...
    if Exlist.GetTableNum(data) == 0 then return end
    if data.weekly and not data.weekly.curr then data.weekly = nil end
    return data
end
...
local data = {
    ...
    updater = Updater,
    event = {
        "PLAYER_ENTERING_WORLD", "AZERITE_ITEM_EXPERIENCE_CHANGED",
        "AZERITE_ITEM_POWER_LEVEL_CHANGED",
        "AZERITE_EMPOWERED_ITEM_SELECTION_UPDATED", "PLAYER_EQUIPMENT_CHANGED",
        "QUEST_LOG_UPDATE"
    },
    weeklyReset = true,
    dailyReset = false,
